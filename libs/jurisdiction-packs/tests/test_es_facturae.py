...
from jurisdiction_packs.es.facturae import generate_xml, parse_structured
...
def _sample_invoice() -> CanonicalInvoice:
    return CanonicalInvoice(
        invoice_key="company-1:B12345674:F001-1", company_id=uuid4(), jurisdiction="ES",
        supplier_tax_id="B12345674", series_number="F001-1", issue_date=date(2026, 1, 15),
...
def test_generated_xml_uses_the_real_facturae_namespace():
    xml = generate_xml(_sample_invoice())
    assert b"facturae.es/Facturae" in xml
        ...
        update={
            "lines": [
                InvoiceLine(
                    ...
                    unit_price=Money(amount=unit_price, currency="EUR"),
                    line_total=Money(amount=line_total, currency="EUR"),
                )
            ]
        }
    )
